...
use App\Models\Backend\ProductCate;
    ...
    */
    ...
    public function mapThuocTinhTablet(){
        ...
        foreach($arr as $value){
            ...
            $rs = Product::where('external_id', $external_id)->get()->toArray();
            if(!empty($rs)){
                $id = $rs[0]['id'];
        ...
        foreach ($data as $product_id => $arrValue) {
            echo $product_id;
            echo "<hr>";
    ...
    public function mapThuocTinhLcd(){
        ...
        foreach ($arrData as $product_id => $arrValue) {
            echo $product_id;
            echo "<hr>";
            $model = new SpThuocTinh;
            $model->product_id = $product_id;
    ...
    public function processPrice( ){
        set_time_limit(10000);
        $arrSP = Product::lists('external_id')->toArray();
            ...
            if( $v['meta_key'] == '_product_image_gallery' && $v['meta_value'] != ''){
                ...
                if( !empty($tmp1) ){
                    foreach ($tmp1 as $p_id) {
                        ...
                        if(isset($wpPost[0])){
                            $guid = str_replace("http://www.gfamily.vn/wp-content/uploads/", "", $wpPost[0]['guid']);
        ...
        if( $thumbnail_id > 0){
            if( isset( $data['img']) && !empty($data['img'])){
                ...
                foreach ($imgArr['url'] as $key => $value) {
                    ...
                    $dataHinh['product_id'] = $id;
                    $dataHinh['image_url'] = $value;
                    $dataHinh['display_order'] = 1;
                    $rs = ProductImg::create($dataHinh);
                    $id_hinh = $rs->id;
    ...
    public function chuyenProductCate($parent_id){
        $arrLoai = 
        [
            6 => 2,            
            30 => 8,
        ...
        return isset($arrLoai[$parent_id]) ? $arrLoai[$parent_id] : 0;
    ...
    public function mapProduct(){
        ...
        foreach ($tmp as $key => $value) {
           ...
           */
            ...
            $data['parent_id'] = $this->chuyenProductCate( $value['parent']);
            $data['created_at'] = $value['post_date'];
            $data['updated_at'] = $value['post_modified'];
