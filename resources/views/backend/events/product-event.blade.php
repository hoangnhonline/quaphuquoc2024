    ...
    <div class="col-md-12">
      ...
      <div class="box">
        ...
        <div class="box-body">
          ...
          <table class="table table-bordered" id="table-list-data">
            ...
            @if( $dataList->count() > 0 )
              ...
              @foreach( $dataList as $item )
                <?php $i ++;
                $str_product_id.= $item->product_id.",";
                 ?>
              <tr id="row-{{ $item->id }}">                         
                ...
                <td>                  
                  <a style="color:#333;font-weight:bold" target="_blank" href="{{ route( 'product.edit', [ 'id' => $item->product_id ]) }}">{{ $item->name }}</a> &nbsp; @if( $item->is_hot == 1 )
                ...
                <td>
                  <input class="form-control" type="text" name="so_luong[{{ $item->product_id }}]" value="{{ $item->so_luong }}">
                </td>
                ...
                <td>                                 
                  ...
                  <a onclick="return callDelete('{{ $item->name }}','{{ route( 'events.destroy-product', [ 'product_id' => $item->product_id, 'event_id' => $detail->id ]) }}');" class="btn-sm btn btn-danger">Xóa</a>
          ...
          <input type="hidden" name="str_product_id" id="str_product_id" value="{{ $str_product_id }}" >
...
$(document).ready(function(){
  $(document).on('click', '.checkSelect', function(){  
      var obj = $(this);
      var str_product_id = $('#str_product_id').val();
      if(obj.prop('checked') == true){
        str_product_id += obj.val() + ',';
      }else{
        var str = obj.val() + ',';
        str_product_id = str_product_id.replace(str, '');
      }
      $('#str_product_id').val(str_product_id);      
  });
  $('.btnLienQuan').click(function(){
    $('#label-search').html("sản phẩm khuyến mãi");
    filterAjax();
  }); 
  $(document).on('change', '#parent_id_search, #cate_id_search', function(){
    filterAjax();
  });
...
function filterAjax(){  
  ...
  $.ajax({
          ...
          success: function (response) {
            ...
            var str_checked = $('#str_product_id').val();
