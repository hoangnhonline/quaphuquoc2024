...
class CrawlerController extends Controller
{
    ...
    public function index(Request $request){
        ...
        foreach ($dataArr as $key => $value) {
            ...
            if($site == "tiki"){
                ...
                $dataArr = $this->tiki($url, $value->parent_id, $value->cate_id, $value->type);              


            }elseif($site == "adayroi"){

                $dataArr = $this->adayroi($url, $value->parent_id, $value->cate_id, $value->type);                
            }
            elseif($site == "sendo"){

                $dataArr = $this->sendo($url, $value->parent_id, $value->cate_id, $value->type);
               
            }else{
               
                $dataArr = $this->lazada($url, $value->parent_id, $value->cate_id, $value->type);
    ...
    public function lazada($url, $parent_id, $cate_id, $type){
        
        set_time_limit(10000);
        $arr = ['parent_id' => $parent_id, 'cate_id' => $cate_id, 'type' => $type];
    ...
    public function adayroi($url, $parent_id, $cate_id, $type){        
        set_time_limit(10000);
        $arr = ['parent_id' => $parent_id, 'cate_id' => $cate_id, 'type' => $type];
        for($page = 1; $page <= 1; $page++){ 
    ...
    public function tiki($url, $parent_id, $cate_id, $type){
        $arr = [];
        set_time_limit(10000);
        for($page = 1; $page <= 1; $page++){ 
